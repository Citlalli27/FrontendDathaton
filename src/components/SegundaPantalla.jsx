...
function SegundaPantalla () {
 ...
 const handleInputChange = ( key, event) => {
  ...
  useEffect(() => {
   ...
   const getData = () => {
    axios.get('http://127.0.0.1:5000/savings?client_id=' + currentUserId)
      .then((response) => {
  ...
  return (
    <div> 
      <Header />
       <div class="bg-[#D6D5FF] py-10 md:px-52 flex items-center gap-0 p-1 md:flex-row sm:flex-col flex-col" >
        <h1 class="bg-[#D6D5FF] md:text-3xl sm:text-2xl text-1xl">Establece tu objetivo de ahorro al mes</h1>
      ...
      <h1 className= 'bg-[#D6D5FF] flex justify-center text-[#E53EE3] items-center md:text-2xl sm:text-1xl text-1xl font-bold py-5'> ¡Ahorra más y haz que tu dinero trabaje para ti!  </h1>
      <h1 className= 'bg-[#D6D5FF] flex justify-center items-center md:text-1xl sm:text-0.5xl text-0.5 xl  py-5'> Elige sabiamente las categorías en las en las que deseas reducir la cantidad promedio de tus transacciones y cuida tu salud financiera. </h1>
      
      <div className='w-full flex flex-wrap '>

        <CategoriaContainer> 
          <p className='text-center md:text-2xl font-semibold py-5 p-4 my-0 color-black'> Gastos Mensuales</p>
         ...
         <p className='bg-[#EAF8AE] text-center md:text-2xl font-semibold py-5 p-4 my-0 color-black'> Plan de Ahorro</p>
         <div className=' bg-[#EAF8AE] w-full flex text-center justify-end md:text-base sm:text-base sm:gap-4 gap-5 '>
          ...
          savingMods != null ? Object.keys(savingMods).map((cat) =>  <div className='bg-[#EAF8AE] w-full py-4 flex'>
    <button className= "bg-[#B9B8FF] text-xs w-full  md:w-1/2  text-white py-3.5 px-2 mx-1 rounded  pointer-events-none"> {cat}  </button>
    ...
    <button className= "bg-white text-xs w-full md:w-1/4  text-black  py-2 px-2 mx-1 rounded  pointer-events-none "> {savingMods[cat]["monto"]}  </button>
